...
exports.postAuth = async (request, response) => {
  ...
  try{
    // 이메일 여부
    const rows = await database.pool.query('SELECT * FROM users WHERE email = $1', [email]); // rows = [rows{}]

    // 이메일이 존재하면 메시지 반환
    if (rows.rows.length > 0) {
      return response.status(200).json({ message: "이미 존재하는 이메일입니다.", success: false });
    }
    ...
    await database.pool.query('INSERT INTO users (username, email, password, profile_image) VALUES ($1, $2, $3, $4)', [username, email, hashPassword, profileImagePath]);

    return response.status(201).json({ message: "회원가입 성공", success: true });

  } catch (error) {
    return response.status(500).json({ message: "데이터 입력 오류:" + error.message});
  }
}
...
exports.postLogin = async (request, response) => {
  ...
  try {
    const rows = await database.pool.query('SELECT * FROM users WHERE email = $1', [email]);

    if (rows.rows.length === 0) {
      return response.status(200).json({ message: "존재하지 않는 이메일입니다.", success: false });
    }

    const isMatch = await bcrypt.compare(password, rows.rows[0].password)

    if(!isMatch) {
      return response.status(200).json({ message: "비밀번호가 일치하지 않습니다.", success: false });
    }
